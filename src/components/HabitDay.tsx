...
export function HabitDay({amountOfHabits = 0, amountCompleted = 0, date, ...rest}: Props){
    ...
    const today = dayjs().startOf('day').toDate();    // importar a data atual para deixar o quadradinho do dia em destaque 
    const isCurrentDay = dayjs(date).isSame(today);   // comparando os dias para encontrar o dia atual no array
    ...
    return(
        <TouchableOpacity
            className={clsx("rounded-lg border-2 m-1", {
                ...
                ["border-x-white border-2"] : isCurrentDay
